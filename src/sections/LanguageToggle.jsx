...
function LanguageToggle({ language, onLanguageChange, t }) {
  ...
  const activeLabel = displayNames[language] ?? activeLanguage.label
  ...
  return (
    <motion.section
      ...
      className="particle-field border-y border-emerald-100/80 bg-[radial-gradient(circle_at_top_left,#dcfce7,transparent_28%),linear-gradient(135deg,#ffffff_0%,#f8fafc_48%,#ecfdf5_100%)] px-4 py-12 text-slate-900 sm:px-6 sm:py-16 md:px-8 lg:py-24"
    >
      <div className="section-reveal mx-auto grid max-w-6xl gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        ...
        <motion.div
          variants={cardReveal}
          className="glass-panel glow-card min-w-0 rounded-[1.5rem] p-3 sm:rounded-[2rem] sm:p-5 lg:float-soft"
        >
          <div className="relative flex flex-col gap-4 rounded-[1.5rem] border border-emerald-100/80 bg-emerald-50/70 p-4 shadow-inner shadow-white/50 backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between">
              ...
              <AnimatePresence>
                {isOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-[calc(100%+0.5rem)] z-20 grid w-full gap-1 rounded-3xl border border-emerald-100 bg-white/95 p-2 shadow-2xl shadow-emerald-950/10 backdrop-blur-sm sm:w-72"
                  >
                    {languageOptions.map((option) => (
                      ...
                      >
                        <span>{displayNames[option.code] ?? option.label}</span>
          ...
          <div className="glass-panel-dark mt-5 overflow-hidden rounded-[1.5rem] p-4 text-white sm:p-6">
